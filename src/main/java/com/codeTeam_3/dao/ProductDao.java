...
public class ProductDao {
    ...
    // Thêm method này vào class ProductDao

    public List<ProductView> searchProducts(String keyword, String lang) {
        String sql = """
        SELECT p.ProductID, p.Price, p.Weight,
               pt.ProductName, pt.ProductDescription,
               pct.CategoryName
    ...
    """;
        ...
        try (Connection conn = dbMysql.getConnection();
            ...
            // Xử lý trường hợp tìm theo ID
            int productId = 0;
            try {
                productId = Integer.parseInt(keyword);
            } catch (NumberFormatException e) {
                // Không phải số, để ID = 0
            }
